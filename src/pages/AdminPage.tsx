...
export const AdminPage: React.FC = () => {
  const { isMockAdmin, currentRole, activeUser, loginAsRole, switchRole, signOut } = useAuth();
  const {
    medias: mediasQueue,
  ...
  const handleSupabaseLogin = async (e: React.FormEvent) => {
    ...
    try {
      if (isSupabaseConfigured && supabase) {
        ...
        loginAsRole('super_admin', email || 'admin@ndoh-djuttitsa.cm', 'Super Administrateur');
      }
    } catch (err: any) {
  ...
  // ---------------------------------------------------------------------------
  // ÉCRAN DE CONNEXION AVEC TABLEAU COMPLET DE MOTS DE PASSE DES RÔLES
  // ---------------------------------------------------------------------------
  if (!isAuthenticated) {
    return (
      <div className="min-h-screen bg-slate-50 dark:bg-[#0B0F19] text-slate-900 dark:text-white py-12 px-4 sm:px-6 lg:px-8 transition-colors duration-300">
        ...
        <div className="max-w-6xl mx-auto space-y-10">
          ...
          <div className="text-center space-y-3">
            ...
            <h1 className="text-3xl font-black font-heading tracking-tight text-slate-900 dark:text-white">
              Espace Administrateur & Gestion Rôles
            </h1>
            <p className="text-sm text-slate-600 dark:text-slate-400 max-w-2xl mx-auto">
              Plateforme officielle de la Chefferie Supérieure et de la Communauté de <span className="text-emerald-700 dark:text-emerald-400 font-bold">NDOH-DJUTTITSA</span> (Groupement Bafou, Nkong-Ni, Menoua).
            </p>
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 items-start">
            ...
            <Card className="p-6 sm:p-8 space-y-6 border-slate-200 dark:border-slate-800 bg-white dark:bg-[#111827] shadow-xl">
              <div className="space-y-1">
                <h2 className="text-xl font-bold font-heading text-slate-900 dark:text-white flex items-center gap-2">
                  <Lock className="w-5 h-5 text-emerald-600" />
                  <span>Connexion Sécurisée</span>
                </h2>
                <p className="text-xs text-slate-500">Saisissez vos identifiants ou cliquez sur un rôle à droite.</p>
              </div>
              ...
              <form onSubmit={handleSupabaseLogin} className="space-y-4">
                ...
                <Button
                  ...
                  disabled={loggingIn}
                  size="lg"
                  className="w-full bg-emerald-700 hover:bg-emerald-800 dark:bg-emerald-600 text-white shadow-md"
            ...
            {/* SECTION MOTS DE PASSE DES 6 RÔLES - IMPLÉMENTATION DEMANDÉE */}
            <div className="lg:col-span-2 space-y-4">
              <div className="p-4 rounded-2xl bg-amber-500/10 border border-amber-500/30 flex items-center justify-between gap-4">
                <div className="flex items-center gap-3">
                  <KeyIcon className="w-6 h-6 text-amber-600 dark:text-amber-400 shrink-0" />
                  <div>
                    <h3 className="font-bold text-sm text-slate-900 dark:text-white">Identifiants & Mots de passe de Tous les Rôles</h3>
                    <p className="text-xs text-slate-600 dark:text-slate-400">Cliquez sur n importe quel compte ci-dessous pour tester l interface en tant que ce rôle.</p>
                  </div>
                </div>
                <Badge variant="amber" className="shrink-0 font-mono">6 Rôles Opérationnels</Badge>
              </div>

              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                {MOCK_USER_ROLES.map((r) => (
                  <Card key={r.role} className="p-4 bg-white dark:bg-[#111827] border-slate-200 dark:border-slate-800 space-y-3 hover:border-emerald-500/40 transition-all shadow-sm">
                    <div className="flex items-start justify-between gap-2">
                      <div>
                        <div className="flex items-center gap-2">
                          <h4 className="font-bold text-sm text-slate-900 dark:text-white">{r.title}</h4>
                        </div>
                        <span className="text-[11px] text-slate-500 font-medium">{r.nom}</span>
                      </div>
                      {getRoleBadge(r.role)}
                    </div>

                    <div className="p-2.5 rounded-xl bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 font-mono text-xs space-y-1">
                      <div className="flex justify-between text-slate-600 dark:text-slate-400">
                        <span>Email:</span>
                        <span className="font-bold text-slate-900 dark:text-slate-200">{r.email}</span>
                      </div>
                      <div className="flex justify-between text-amber-700 dark:text-amber-400">
                        <span>Mot de passe:</span>
                        <span className="font-bold bg-amber-500/10 px-1.5 py-0.5 rounded border border-amber-500/30">{r.password}</span>
                      </div>
                    </div>

                    <p className="text-[11px] text-slate-600 dark:text-slate-400 line-clamp-2">{r.description}</p>

                    <div className="pt-2 flex items-center gap-2">
                      <button
                        onClick={() => loginAsRole(r.role, r.email, r.nom)}
                        type="button"
                        className="flex-1 py-1.5 px-3 rounded-xl bg-emerald-700 hover:bg-emerald-800 text-white font-bold text-xs flex items-center justify-center gap-1.5 transition-colors shadow-sm"
                      >
                      ...
                      <button
                        onClick={() => {
                          ...
                          showToast(`Champs remplis pour ${r.title}`);
                        }}
                        type="button"
    ...
    <div className="min-h-screen bg-slate-50 dark:bg-[#0B0F19] text-slate-900 dark:text-white pb-20 transition-colors duration-300">
      ...
      <div className="border-b border-slate-200 dark:border-slate-800 bg-white dark:bg-[#111827] px-4 sm:px-8 py-5 shadow-sm">
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
          ...
          {/* CHANGER DE RÔLE EN DIRECT + BOUTON DÉCONNEXION */}
          <div className="flex flex-wrap items-center gap-3 w-full md:w-auto justify-end">
            <div className="flex items-center gap-2 bg-slate-100 dark:bg-slate-800 p-1.5 rounded-xl border border-slate-200 dark:border-slate-700">
              <RefreshCw className="w-3.5 h-3.5 text-slate-500 ml-1" />
              <span className="text-[11px] font-bold text-slate-600 dark:text-slate-400 hidden sm:inline">Rôle Test:</span>
              <select
                value={currentRole}
                onChange={(e) => {
                  const r = e.target.value as UserRole;
                  switchRole(r);
                  showToast(`Session basculée sur le rôle ${r.toUpperCase()}`);
                }}
                className="px-2.5 py-1 rounded-lg bg-white dark:bg-slate-900 text-slate-900 dark:text-white font-bold text-xs border border-slate-300 dark:border-slate-700 focus:outline-none focus:ring-2 focus:ring-emerald-500"
              >
                <option value="super_admin">1. Super Admin</option>
                <option value="administrateur">2. Administrateur</option>
                <option value="redacteur">3. Rédacteur</option>
                <option value="moderateur">4. Modérateur</option>
                <option value="archiviste">5. Archiviste</option>
                <option value="habitant">6. Habitant (Restreint)</option>
              </select>
            </div>
