...
export const AdminPage: React.FC = () => {
  ...
  if (!isAuthenticated) {
    return (
      <div className="min-h-[80vh] flex items-center justify-center px-4 py-16 bg-slate-950">
        <Card className="w-full max-w-md p-8 space-y-6 border-slate-800 bg-slate-900 text-white shadow-2xl">
          <div className="text-center space-y-3">
            ...
            <h2 className="text-2xl font-extrabold font-heading text-white">Espace Administrateur</h2>
            <p className="text-xs text-slate-400">
              Accès réservé au Super Administrateur de la plateforme NDOH-DJUTTITSA.
            </p>
          </div>
          ...
          <form onSubmit={handleSupabaseLogin} className="space-y-4">
            <div className="space-y-1.5">
              <label className="text-xs font-bold text-slate-300">Adresse Email Super Admin</label>
              <input
                type="email"
                required
                value={email}
          ...
          <div className="pt-4 border-t border-slate-800 text-center space-y-3">
            <p className="text-xs text-slate-500">Ou tester immédiatement le panneau sans Supabase réel :</p>
            <Button
              onClick={loginAsDemoAdmin}
              variant="outline"
              size="sm"
              className="w-full border-amber-500/40 text-amber-300 hover:bg-amber-950/50"
            >
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-white pb-20">
      <div className="border-b border-slate-800 bg-slate-900/80 px-4 sm:px-8 py-6">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="flex items-center gap-2">
                <h1 className="text-xl font-bold font-heading text-white">Console Super Admin</h1>
                <Badge variant="emerald">{isMockAdmin ? 'Mode Démo Admin' : 'Supabase Auth JWT'}</Badge>
              </div>
              <p className="text-xs text-slate-400">Gestion dynamique des contenus et RLS Security</p>
            </div>
          </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-8 pt-8 space-y-8">
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          <div className="p-5 rounded-2xl bg-slate-900 border border-slate-800 space-y-1">
            ...
            <div className="text-2xl font-extrabold font-heading text-white">{actualitesList.length}</div>
            <div className="text-xs text-slate-400">Actualités enregistrées</div>
          </div>
          <div className="p-5 rounded-2xl bg-slate-900 border border-slate-800 space-y-1">
            <Calendar className="w-5 h-5 text-amber-400 mb-2" />
            <div className="text-2xl font-extrabold font-heading text-white">{MOCK_EVENEMENTS.length}</div>
            <div className="text-xs text-slate-400">Événements programmés</div>
          </div>
          <div className="p-5 rounded-2xl bg-slate-900 border border-slate-800 space-y-1">
            <ImageIcon className="w-5 h-5 text-blue-400 mb-2" />
            <div className="text-2xl font-extrabold font-heading text-white">{MOCK_MEDIAS.length}</div>
            <div className="text-xs text-slate-400">Fichiers médias & Galerie</div>
          </div>
          <div className="p-5 rounded-2xl bg-slate-900 border border-slate-800 space-y-1">
            <Mail className="w-5 h-5 text-purple-400 mb-2" />
            <div className="text-2xl font-extrabold font-heading text-white">4</div>
            <div className="text-xs text-slate-400">Messages contacts reçus</div>
          </div>
        </div>
        ...
        <div className="flex flex-wrap gap-2 p-1.5 bg-slate-900 rounded-2xl border border-slate-800">
          <button
            ...
            className={`px-4 py-2.5 rounded-xl font-bold text-xs transition-all ${
              adminTab === 'actualites' ? 'bg-emerald-600 text-white' : 'text-slate-400 hover:text-white'
            }`}
          >
          ...
          <button
            ...
            type="button"
            className={`px-4 py-2.5 rounded-xl font-bold text-xs transition-all ${
              adminTab === 'evenements' ? 'bg-emerald-600 text-white' : 'text-slate-400 hover:text-white'
            }`}
          >
          ...
          <button
            ...
            type="button"
            className={`px-4 py-2.5 rounded-xl font-bold text-xs transition-all ${
              adminTab === 'medias' ? 'bg-emerald-600 text-white' : 'text-slate-400 hover:text-white'
            }`}
          >
            Stockage & Médias
          </button>
          <button
            ...
            type="button"
            className={`px-4 py-2.5 rounded-xl font-bold text-xs transition-all ${
              adminTab === 'sql' ? 'bg-amber-600 text-white' : 'text-slate-400 hover:text-white'
            }`}
          >
        ...
        {adminTab === 'actualites' && (
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            <Card className="p-6 bg-slate-900 border-slate-800 space-y-4">
              ...
              <form onSubmit={handleAddNews} className="space-y-3 text-xs">
                <div className="space-y-1">
                  <label className="font-bold text-slate-300">Titre de l article *</label>
                  <input
                    type="text"
                    required
                    value={newTitle}
                ...
                <div className="space-y-1">
                  <label className="font-bold text-slate-300">URL de l image (Optionnelle)</label>
                  <input
                    type="text"
                    value={newImageUrl}
                ...
                <div className="space-y-1">
                  <label className="font-bold text-slate-300">Chapeau / Résumé</label>
                  <input
                    type="text"
                    value={newChapeau}
                ...
                <div className="space-y-1">
                  <label className="font-bold text-slate-300">Contenu complet *</label>
                  <textarea
                    required
                    rows={4}
                    value={newContent}
            ...
            <div className="lg:col-span-2 space-y-4">
              <h3 className="text-lg font-bold font-heading text-white">Articles publiés dans le système</h3>
                  ...
                  >
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <h4 className="font-bold text-sm text-white line-clamp-1">{item.titre}</h4>
          ...
          <Card className="p-8 bg-slate-900 border-slate-800 space-y-6">
            <div className="space-y-2">
              <div className="flex items-center gap-2">
                ...
                <h3 className="text-xl font-bold font-heading text-white">Instructions de Déploiement & SQL Supabase</h3>
              </div>
              <p className="text-xs text-slate-400">
                Pour connecter votre instance Supabase réelle à cette plateforme, créez les tables et politiques RLS en exécutant le fichier <code className="text-amber-300">supabase_schema.sql</code> généré à la racine du projet.
              </p>
            </div>

            <div className="p-4 rounded-2xl bg-slate-950 border border-slate-800 text-xs font-mono text-emerald-300 space-y-2">
